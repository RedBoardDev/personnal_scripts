...
then
    ...
    if [ $GHCR_REPOSITORY_STATUS -eq 0 ]; then
        LOCAL_IMAGE_DIGEST=$($BASE_EXEC_CMD images --digests --no-trunc --quiet ghcr.io/epitech/coding-style-checker:latest | grep -o 'sha256:[0-9a-f]*')
        REMOTE_IMAGE_DIGEST=$(curl -s -H "Authorization: Bearer $GHCR_REGISTRY_TOKEN" "https://ghcr.io/v2/epitech/coding-style-checker/manifests/latest" | jq -r '.config.digest')
        if [ "$LOCAL_IMAGE_DIGEST" != "$REMOTE_IMAGE_DIGEST" ]; then
            echo "New image available, downloading and cleaning old one..."
            $BASE_EXEC_CMD pull ghcr.io/epitech/coding-style-checker:latest && $BASE_EXEC_CMD image prune -f > /dev/null 2>&1
            echo "Download OK"
        else
            echo "No updates found. Skipping image download."
        fi
    else
    ...
    # $BASE_EXEC_CMD run --l -v "$DELIVERY_DIR":"/mnt/delivery" -v "$REPORTS_DIR":"/mnt/reports" ghcr.io/epitech/coding-style-checker:latest "/mnt/delivery" "/mnt/reports"
    ...
    TEMP_REPORTS_FILE=$(mktemp)
    $BASE_EXEC_CMD run --rm -i -v "$DELIVERY_DIR":"/mnt/delivery" -v "$TEMP_REPORTS_FILE":"/mnt/reports/coding-style-reports.log" ghcr.io/epitech/coding-style-checker:latest "/mnt/delivery" "/mnt/reports"
    cat "$TEMP_REPORTS_FILE"
    rm "$TEMP_REPORTS_FILE"
else
    cat_readme
fi
